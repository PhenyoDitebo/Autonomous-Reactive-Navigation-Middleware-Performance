...
buffer=0.2 # (to avoid collisions due to momentum issues)
...
# -------------------------------------- FUNCTIONS --------------------------------------
move_forward() {
    # find how long to move based on safe distance
    safe_distance=$(echo "$ff_range - $safety_margin" | bc -l)
    
    # if safe distance is positive, calculate sleep time
    if (( $(echo "$safe_distance > 0" | bc -l) )); then
        sleep_time=$(echo "$safe_distance / $forw_speed" | bc -l)
    else
        # If we're already at/past safe distance, don't move
        sleep_time=0
    fi
    
    # set velocity and move for calculated time
...
move_backward() {
    # find how long to back up based on available space behind
    safe_distance=$(echo "$bb_range - $safety_margin" | bc -l)
    
    if (( $(echo "$safe_distance > 0" | bc -l) )); then
        # use -ve backspeed since its -ve velocity
        sleep_time=$(echo "$safe_distance / -$back_speed" | bc -l)
    else
        sleep_time=0.0
    fi
